...
class DConv(nn.Module):
    def __init__(self, inp_dim, out_dim, kernel_size=3, stride=1, bn=False, relu=True):
        ...
        self.conv = DeformableConv2d(inp_dim, out_dim, kernel_size, stride, padding=(kernel_size - 1) // 2, bias=True)
        self.relu = None
        self.bn = None
...
class Residual_dcn(nn.Module):
    def __init__(self, inp_dim, out_dim):
        super(Residual_dcn, self).__init__()
        self.relu = nn.ReLU()
        self.bn1 = nn.BatchNorm2d(inp_dim)
        self.conv1 = DConv(inp_dim, int(out_dim / 2), 1, relu=False)  # kernel= 1, stride=1
        self.bn2 = nn.BatchNorm2d(int(out_dim / 2))
        self.conv2 = DConv(int(out_dim / 2), int(out_dim / 2), 3, relu=False)
        self.bn3 = nn.BatchNorm2d(int(out_dim / 2))
        self.conv3 = DConv(int(out_dim / 2), out_dim, 1, relu=False)
        self.skip_layer = DConv(inp_dim, out_dim, 1, relu=False)
        if inp_dim == out_dim:
            self.need_skip = False
        else:
            self.need_skip = True
...
class Hourglass_dcn(nn.Module):
    def __init__(self, n, f, bn=None, increase=0):
        super(Hourglass_dcn, self).__init__()
        nf = f + increase
        self.up1 = Residual_dcn(f, f)
        # Lower branch
        self.pool1 = Pool(2, 2)
        self.low1 = Residual_dcn(f, nf)
        self.n = n
        # Recursive hourglass
        if self.n > 1:
            self.low2 = Hourglass(n - 1, nf, bn=bn)
        else:
            self.low2 = Residual_dcn(nf, nf)
        self.low3 = Residual_dcn(nf, f)
