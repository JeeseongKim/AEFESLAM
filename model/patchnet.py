...
class Quad_L2Net(PatchNet):
    ...
    def __init__(self, dim=128, mchan=4, relu22=False, **kw):
        PatchNet.__init__(self, **kw)
        self.add_conv = lambda n, **kw: self._add_conv((n * dim) // 128, **kw)
        self.layer1 = self.add_conv(8 * mchan)
        self.layer2 = self.add_conv(8 * mchan)
        self.layer3 = self.add_conv(16 * mchan, stride=2)
        self.layer4 = self.add_conv(16 * mchan)
        self.layer5 = self.add_conv(32 * mchan, stride=2)
        self.layer6 = self.add_conv(32 * mchan)
        # replace last 8x8 convolution with 3 2x2 convolutions
        self.layer7 = self.add_conv(32 * mchan, k=2, stride=2, relu=relu22)
        self.layer8 = self.add_conv(32 * mchan, k=2, stride=2, relu=relu22)
        self.layer9 = self.add_conv(dim, k=2, stride=2, bn=False, relu=False)
        self.out_dim = dim
