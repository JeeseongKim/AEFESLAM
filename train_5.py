...
def train():
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            attention_map, Hk = model_DETR(aefe_input)
            tf_attention_map, tf_Hk = model_DETR(tf_aefe_input)

            MaxPool1d_3 = torch.nn.MaxPool1d(3)
            attention_score = torch.sum(attention_map, dim=1).unsqueeze(1)
            pooled_attention = 0.1*MaxPool1d_3(attention_score)

            tf_attention_score = torch.sum(tf_attention_map, dim=1).unsqueeze(1)
            tf_pooled_attention = 0.1*MaxPool1d_3(tf_attention_score)

            # keypoint extraction
            fn_DetectionConfidenceMap2keypoint = getKP_DETR()
            Ok, tf_Ok, Vk, tf_Vk, kp, tf_kp, zeta, tf_zeta = fn_DetectionConfidenceMap2keypoint(Rk, tf_Rk, my_height, my_width, pooled_attention, tf_pooled_attention, Hk, tf_Hk)
            ...
            recon_Wk_2d = model_dec_feature_descriptor(fk)
            recon_tf_Wk_2d = model_dec_feature_descriptor(tf_fk)
