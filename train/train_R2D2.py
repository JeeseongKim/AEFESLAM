...
def train():
    ...
    model_L2 = L2net_R2D2(num_of_kp=num_of_kp)
    model_L2 = nn.DataParallel(model_L2).cuda()
    # optimizer_StackedHourglass_kp = torch.optim.Adam(model_StackedHourglassForKP.parameters(), lr=1e-3, weight_decay=2e-4)
    optimizer_L2_kp = torch.optim.AdamW(model_L2.parameters(), lr=learning_rate, weight_decay=weight_decay)
    ...
    if os.path.exists("./SaveModelCKPT/train_model.pth"):
        ...
        model_L2.module.load_state_dict(checkpoint['model_L2'])
        ...
        optimizer_L2_kp.load_state_dict(checkpoint['optimizer_L2_kp'])
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            time2 = time.time()
            Rk = model_L2(aefe_input)
            tf_Rk = model_L2(tf_aefe_input)
            ...
            Dk, kp, zeta, tf_Dk, tf_kp = fn_DetectionConfidenceMap2keypoint(Rk.clone(), tf_Rk, cur_batch)
            # print("KP extract", time.time()-time3)
            Dk_con_loss = loss_concentration(Dk).cuda()
            ...
            reconRk_con_loss = loss_concentration(reconRk_f).cuda()
            reconDk_con_loss = loss_concentration(reconDk_kp).cuda()

            cur_con_loss = Dk_con_loss() + Rk_con_loss() + reconRk_con_loss() + reconDk_con_loss()
            ...
            loss = my_sep_loss + my_cosim_loss + my_transf_loss + my_dk_loss + my_rk_loss + my_recon_loss + my_con_loss

            print("Sep: ", my_sep_loss.item(), ", Con: ", my_con_loss.item(), ", Cosim: ", my_cosim_loss.item(), ", Trans: ", my_transf_loss.item(), ", Dk: ", my_dk_loss.item(), ", Rk: ", my_rk_loss.item(), ", Recon:", my_recon_loss.item())
            # print("loss compute", time.time() - time11)
            # ================Backward================
            time12 = time.time()
            optimizer_L2_kp.zero_grad()
            ...
            if (((epoch + 1) % 5 == 0) or (epoch == 0) or (epoch + 1 == num_epochs)):
                ...
                if (epoch != 0):
                    torch.save({
                        'model_L2': model_L2.module.state_dict(),
                        ...
                        'optimizer_L2_kp': optimizer_L2_kp.state_dict(),
        ...
        saveLossData = 'epoch\t{}\tAll_Loss\t{:.4f} \tRecon\t{:.4f} \tCosim\t{:.4f}\tSep\t{:.4f} \tTrans\t{:.4f}\tDk\t{:.4f}\tRk\t{:.4f}\tConc\t{:.4f}\n'.format(
            epoch, running_loss, running_recon_loss, running_cosim_loss, running_sep_loss, running_transf_loss,
            running_detection_loss, running_score_loss, running_con_loss)
