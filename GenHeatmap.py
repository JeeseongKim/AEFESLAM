...
class L2net_R2D2(nn.Module):
    def __init__(self, num_of_kp):
        ...
        self.conv128_kp = nn.Conv2d(in_channels=128, out_channels=num_of_kp, kernel_size=3, stride=1, padding=1, dilation=1, bias=True)
        self.conv_kp_kp_1 = nn.Conv2d(in_channels=num_of_kp, out_channels=num_of_kp, kernel_size=2, stride=1, padding=1, dilation=2, bias=True)
        self.conv_kp_kp_2 = nn.Conv2d(in_channels=num_of_kp, out_channels=num_of_kp, kernel_size=2, stride=1, padding=1, dilation=2, bias=True)
        self.conv_kp_kp_3 = nn.Conv2d(in_channels=num_of_kp, out_channels=num_of_kp, kernel_size=2, stride=1, padding=1, dilation=2, bias=True)
    ...
    def forward(self, aefe_input):
        ...
        #out = F.relu(out)
        out = self.conv128_kp(out)
        #out = F.relu(out)
        #out = self.conv_kp_kp_1(out)
        #out = F.relu(out)
        #out = self.conv_kp_kp_2(out)
        #out = F.relu(out)
        out = self.conv_kp_kp_3(out)
        #out = F.relu(out)
