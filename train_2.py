...
from CnnKPNetwork import CNNForKP
...
my_width = 272 #96 #272 #208
my_height = 80 #32 #80 #64
...
def train():
    ...
    model_cnn_ForKP = CNNForKP(inp_h=my_height, inp_w=my_width, num_of_kp=num_of_kp)
    model_cnn_ForKP = nn.DataParallel(model_cnn_ForKP).cuda(0)
    optimizer_cnn_ForKP = torch.optim.Adam(model_cnn_ForKP.parameters(), lr=learning_rate, weight_decay=weight_decay)
    ...
    ###################################################################################################################
    if os.path.exists("./SaveModelCKPT2/train_model.pth"):
        checkpoint = torch.load("./SaveModelCKPT2/train_model.pth")
        model_cnn_ForKP.module.load_state_dict(checkpoint['model_StackedHourglassForKP'])
        ...
        optimizer_cnn_ForKP.load_state_dict(checkpoint['optimizer_StackedHourglass_kp'])
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            combined_hm_preds = model_cnn_ForKP(aefe_input) #Rk
            tf_combined_hm_preds = model_cnn_ForKP(tf_aefe_input) #Rk
            ...
            if (epoch % 5 == 0):
                ...
                img_save_filename = ("SaveReconstructedImg2/recon_%s_epoch_%s.jpg" % (cur_filename, epoch))
                save_image(reconImg, img_save_filename)
                if(epoch != 0):
                    torch.save({
                                'model_StackedHourglassForKP': model_cnn_ForKP.module.state_dict(),
                                ...
                                'optimizer_StackedHourglass_kp': optimizer_cnn_ForKP.state_dict(),
...
if __name__ == '__main__':
    ...
    if not os.path.exists("SaveKPImg2"):
        os.makedirs("SaveKPImg2")
    if not os.path.exists("SaveReconstructedImg2"):
        os.makedirs("SaveReconstructedImg2")
    if not os.path.exists("SaveHeatMapImg2"):
        os.makedirs("SaveHeatMapImg2")
    if not os.path.exists("SaveModelCKPT2"):
        os.makedirs("SaveModelCKPT2")
